...
public class SonarStatsCalculatorServiceTest {
    ...
    public void issuesAfterLegacyDateAreNotProcessed() {
        final Issue issue = createIssue(LocalDate.of(2016, 5, 1), 30, SeverityType.BLOCKER);
    ...
    public void issuesAtLegacyDateAreNotProcessed() {
        final Issue issue = createIssue(LocalDate.of(2016, 5, 20), 30, SeverityType.BLOCKER);
        final SonarStats stats = service.fromIssueList(Collections.singleton(issue));
        assertThat(stats.getTotalPoints()).isZero();
    }
    ...
    public void issuesBeforeLegacyDateAreProcessed() {
        final Issue issue = createIssue(LocalDate.of(2016, 4, 20), 30, SeverityType.BLOCKER);
        final SonarStats stats = service.fromIssueList(Collections.singleton(issue));
    ...
    public void checkStatsNumbers() {
        ...
        final Issue i1 = createIssue(date, 5, SeverityType.MINOR);
        final Issue i2 = createIssue(date, 10, SeverityType.BLOCKER);
        final Issue i3 = createIssue(date, 15, SeverityType.CRITICAL);
        final Issue i4 = createIssue(date, 20, SeverityType.INFO);
        final Issue i5 = createIssue(date, 25, SeverityType.MAJOR);
    ...
    private Issue createIssue(final LocalDate creationDate, final int debtMinutes, final SeverityType severityType) {
